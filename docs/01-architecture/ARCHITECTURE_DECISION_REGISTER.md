...
**Status:** **PROPOSED (draft).** Awaiting product-owner/security acceptance before Stage 1C implementation.
See `docs/build/stages/STAGE_1C_AUTH_SESSIONS_READINESS.md` §10–§11.

**Decision (proposed):** authenticated sessions are **opaque server-side records**, not stateless JWTs. The
...
**Open sub-decision (D3):** transport — **HttpOnly + Secure + SameSite cookie with a CSRF token
(recommended)** vs `Authorization: Bearer`. To be fixed on acceptance.
...
**Decision (proposed):** store password credentials hashed with **Argon2id** (per-credential
