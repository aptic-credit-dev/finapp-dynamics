...
## ADR-111 — M28 Executive Copilot is READ-ONLY, CITED and RLS-MASKED; it never executes a controlled action; GAP-4 is resolved by reusing the ai.* namespace (Stage 5)
**Status:** **ACCEPTED** — 2026-08-08. Module `m28-executive-ai`, branch `feature/stage-5-m28-executive-ai`.

**Decision:** M28 is a **READ-ONLY** executive assistant that answers executive questions and produces **cited** cross-domain summaries (operations, finance, legal, feedback, cases, KPIs, trends, risk, exceptions, portfolio) for MD/CEO/COO/CFO. It **never** mutates a business record, approves, posts, disburses, reconciles, closes a case, files a matter, sends a notification, changes roles/rules/workflow or executes **any** controlled action — a human decides (CLAUDE.md). **Read-only is enforced in five layers:** the pure command/intent gate (`evaluateReadOnlyGate` refuses a mutating/controlled intent, fail closed), the pure prompt-injection screen (`screenPromptInjection` refuses jailbreak/exfiltration/fabrication), the services (every request authorizes an `ai.copilot.*` permission, default deny), the API (no business-mutation route exists; the only writes are the copilot's OWN assistance records), and the database (`copilot_query.read_only` CHECK = true; `copilot_config` can never disable `read_only`/`citations_required`/`require_human_review_for_export`; the app role holds NO grant on any business table). A refused query is durably persisted as `refused` with a machine-readable reason code and no side effect. **Every substantive answer is CITED:** a completed response must carry at least one citation (`copilot_response_cited_ck` — no uncited factual answer, no fabricated citation); a response that cannot be cited or clears policy is `review_required`, never silently completed. **RLS masking / entitlement intersection:** the copilot NEVER expands the caller's authority — a piece of evidence is visible only if it belongs to the caller's tenant, matches the caller's scope (platform requires `ai.copilot.platform`), passes the sensitivity gate (confidential/restricted require `ai.copilot.sensitive`) and the caller holds every entitlement the evidence requires (`evaluateEntitlement`/`maskEvidence`); masked evidence is dropped, never cited and never counted (no hidden-count leakage, no cross-tenant inference). **GAP-4 resolution:** the `/api/v1/copilot` API — which had NO permission namespace — reuses the shared **`ai.*`** namespace with **7 new `ai.copilot.*` codes** (`read`, `query`, `feedback` unprivileged; `export`, `sensitive`, `configure`, `platform` privileged), exactly as m25/m26/m27 reused m24's namespace; no new namespace, no `ai.admin`/`copilot.admin`, no universal bypass. Every route (read AND mutating) authorizes exactly one code server-side (default deny) and is audited under the shared **`AI_`** prefix (9 `AI_COPILOT_*` codes).

**Consequence:** an executive gets fast, evidence-backed, human-verifiable cross-domain answers that never exceed what they are already entitled to see and never touch a business record — provably, in the gate, the services, the API and the database. Rejected: an AI-executed controlled action; a mutating business route on the copilot; an uncited factual answer or a fabricated citation; a caller seeing another tenant's or an over-privileged row; a masked-row count leak; a permission namespace bypass; a config that disables read-only or citations.

## ADR-112 — M28 consumes M24 (gateway) BY CONTRACT and defers the UNBUILT M32 analytics behind a read-only port; cross-domain reads are read-only ports; citations are references, never content (Stage 5)
**Status:** **ACCEPTED** — 2026-08-08. Module `m28-executive-ai`, branch as ADR-111.

**Decision:** M28 owns **7 tables** (`copilot_config/session/query/response/citation/feedback/idempotency`) and **no** AI capability of its own. It consumes the **M24** governed AI pipeline through a **`CopilotAiGatewayPort`** (`M24CopilotGateway` wraps M24's `RequestService`), so provider selection, DLP (restricted data stays blocked), approved-provider routing, confidence, citations and usage/cost all live in **M24** — M28 never selects providers, never touches provider credentials, never bypasses DLP and never writes M24's private tables; it holds only **opaque** M24 request/output ids, a duplicate handoff is prevented by an idempotency ledger, a blocked/rejected M24 request leaves the query durably `failed`, and the copilot NEVER auto-approves the M24 output (the executive who reads the answer is the human reviewer). **M32 analytics is UNBUILT (Stage 6, mvp:false) and is DEFERRED behind a read-only `ExecutiveAnalyticsPort`** — a bounded, tenant-scoped, entitlement-filtered, citation-bearing aggregate port with a deterministic fixture double and a fail-closed `UnavailableAnalyticsPort` (when the real m32 is absent the answer becomes `review_required`, never an uncited guess); **M28 does NOT implement m32**. Cross-domain evidence (finance, operations, legal, cases, documents) is read through **read-only ports** returning bounded, entitlement- and citation-bearing DTOs — no port exposes a write method and none reads another module's private tables directly; until each module's real read adapter is wired, **deterministic offline doubles** stand in (no network, no secret, no production provider), so the copilot remains useful with fixture-backed evidence. A **citation** preserves a source type/module, an **opaque** record/document reference, a document version/hash, a bounded location, a retrieval timestamp, confidence and the entitlement result — **never copied restricted content** (large question/answer content lives behind **M09** references; the question is never stored inline). It shares the **`ai.*`** namespace and the **`AI_`** audit prefix, publishes **no** domain event and owns **no** second outbox (naming-map `event_families: []`) — the AI request/output lifecycle is emitted by M24 through the one m06 outbox. Audit payloads carry safe ids, status, intent class, reason codes, confidence and source **counts** only — never the question, the answer, restricted content, a privileged narrative, secrets or credentials.

**Consequence:** the executive-copilot layer adds a governed, read-only, cited, masked assistance workflow on top of the existing platform without duplicating a single engine, table or outbox, without a second AI/provider/DLP path, without building m32, and without persisting restricted content — a real model, a real DLP and the real m32 analytics drop in behind their ports unchanged. Rejected: a second AI/provider/DLP engine; building m32; a direct provider call or network egress; reading or writing m24/m32/business tables; a citation holding content; the question stored inline; a new event family or a second outbox; an HTTP surface not assigned by the naming-map.

## ADR-113 — M24 owns the ai.governance_lifecycle family; M29 is an authorized EMITTER that reuses it (no new family, no schema fork) (Stage 5)
**Status:** **ACCEPTED** — 2026-08-08. Module `m29-ai-governance`, branch `feature/stage-5-m29-ai-governance`.

**Decision:** The `ai.governance_lifecycle` event family is **declared and OWNED by M24** (`packages/contracts/src/ai-events.ts`; one registry entry in `event-registry.yaml`; the `AiGovernanceLifecycleEvent` arm of the typed `DomainEvent` union). The naming-map co-lists the family for M24 and M29; this ADR resolves that as **canonical owner (M24) + authorized emitter/producer (M29)**. M29 does **not** declare a second event family, does **not** fork the schema, and owns **no** outbox. It EMITS governance/release lifecycle transitions by reusing M24's EXISTING `GovernanceControlUpdated` event type on the **one m06 outbox**; the discriminating detail (record kind — policy/use_case/release/waiver — from/to status, reason code, opaque module/asset refs) travels in the safe `AiLifecyclePayload` (`isAssistive` always true). No change is made to `contracts/` or `event-registry.yaml`, so conformance (which re-derives event-family ownership from `implementation-manifest.yaml` and the contracts union) stays green with the family registered exactly once.

**Consequence:** the AI-governance layer emits a governed lifecycle signal through the single outbox and the single family, without a second family, a schema fork or a duplicate registry entry — a downstream consumer subscribes to `ai.governance_lifecycle` and filters on the payload `recordType`/`toStatus`. Rejected: a new `governance.*`/`ai.governance_release_lifecycle` family; new event types added to M24's family by M29; a second outbox; a governance event carrying prompt/output/restricted content.

## ADR-114 — M29 AI Governance & Release: AI NEVER approves its own release (maker≠checker, human approver, evaluation evidence); it governs M24 by opaque reference and executes no runtime action (Stage 5)
**Status:** **ACCEPTED** — 2026-08-08. Module `m29-ai-governance`, branch as ADR-113.

**Decision:** M29 owns **7 tables** (`ai_governance_policy/use_case/release/evaluation/decision/history/idempotency`) and is the enterprise oversight layer for the AI lifecycle: it governs AI **use cases** + **policies** and the human-approved **RELEASE** of M24 assets (model/prompt/provider/policy/use-case versions), with evaluation **evidence**, controlled **waivers/overrides**, and suspension/withdrawal. **THE LOAD-BEARING RULE — AI NEVER APPROVES ITS OWN RELEASE — is enforced in THREE layers:** the pure gates (`evaluateSodGate`/`evaluateReleaseGate`/`evaluateWaiverGate` + `isHumanActor`: a null/blank/`system`/`ai`/`automation` actor is never a human, and the proposer can never be the approver, fail closed), the services (an explicit human-actor guard + the gate before every approval), and the **database** (`ai_governance_release_human_ck`: approved/released requires a non-null `approved_by`; `ai_governance_release_sod_ck`: `approved_by <> proposed_by`; `ai_governance_release_evidence_ck`: a non-waiver release cannot be approved without `evaluation_passed`; the decision ledger's `decider` is NOT NULL). A **policy** can never disable human approval or evaluation and can never blanket-allow a restricted provider (`ai_governance_policy_human_ck`/`_eval_ck`/`_restricted_ck`); a governed **use case** can never permit an AI-executed controlled action (`ai_governance_use_case_noaction_ck`); an **ABSOLUTE** control (no-production-provider, no-secret, no-restricted-data, no-AI-controlled-action, no-AI-self-approval, human-review) can **never** be waived (`isAbsoluteControl` → `AI_GOVERNANCE_OVERRIDE_BLOCKED`, fail closed). M29 consumes **M24 BY CONTRACT**: it references M24 assets by **opaque uuid** only (reads no m24/business table, calls no provider, stores no credential/secret, holds no prompt/output content) and never selects providers or bypasses DLP. It shares the **`ai.*`** namespace (3 NEW privileged codes `ai.governance.approve`/`override`/`export`; `read`/`manage` were m24's) and the **`AI_`** prefix (16 `AI_GOVERNANCE_*` codes). It has **NO REST API** (`api_prefixes: []` — internal governed library) and executes **NO** domain action and **NO** deployment/runtime control: it records the governed DECISION + EVIDENCE and emits the lifecycle event; actual deployment stays behind downstream/runtime integration. Confidence/accuracy are integer basis points; no float; no secret column; the release/waiver lifecycles run through a single transition choke point with optimistic concurrency + idempotency + append-only evidence.

**Consequence:** the AI lifecycle gains a governed, human-approved release + evaluation-evidence + exception workflow on top of the platform without duplicating M24 (or M25–M28), without a production provider, without a second AI engine/outbox/event family, and — provably, in the gates, the services and the database — with no path by which the AI approves its own release, waives an absolute control, or executes a controlled action. Rejected: an AI/self-approved release; a release approved with no passing evaluation; a policy disabling human approval/evaluation or allowing a restricted provider; a use case permitting an AI-executed action; a waived absolute control; a direct provider call, credential store or M24 private-table read; a REST surface or runtime deployment action; a second engine, outbox or event family.

## ADR-115 — M30 Platform Foundation is the canonical owner of platform metadata/config/feature-flags/secret-references; M04 stays the admin console (GAP-5 resolved, no duplicate engine); `platform.*` namespace closes GAP-2 (Stage 6A)
**Status:** **ACCEPTED** — 2026-08-08. Module `m30-platform`, branch `feature/stage-6-m30-platform`.

**Decision (GAP-5 — feature-flag / platform-capability ownership):** repository inspection proves **feature flags are not implemented anywhere** — `m04-admin` is an ADMIN CONSOLE / ORCHESTRATION-ONLY module (Stage 1) that owns only its four admin-state tables (saved views, preferences, admin-operation request/history) and CALLS other modules' public services; it has **no feature-flag data model or engine** (the only `feature` table in the repo is `finance_ai_feature`, m27's explainability table — unrelated). The GAP-5 conflict was a documentation overlap, not two engines. **Resolution:** **M30 is the canonical owner** of the platform-foundation capabilities — platform **metadata**, governed **configuration** (definitions + values + history), **feature flags** (definitions + assignments + evaluation + history) and **secret references**; **M04 remains the admin console** and will orchestrate over M30's public service contracts (consume by contract), never own a competing engine. There is **ONE** feature-flag engine (M30), no duplicate. `SHARED_SERVICE_OWNERSHIP.md` is updated to record M30 as the canonical owner (it already assigned secret *references* to m30). **A feature flag is NEVER an authorization substitute:** evaluation is independent of RBAC; a flag cannot grant a permission RBAC denies (RBAC (m02) remains authoritative — `RBAC DENY + FEATURE ENABLED = DENY`, tested explicitly); a **platform-absolute** feature/config cannot be weakened by a tenant override.

**Decision (GAP-2 — permission namespace):** M30 had `permission_namespaces: []`. **Resolution:** a new three-segment **`platform.*`** namespace — `platform.metadata.read/manage`, `platform.config.read/manage`, `platform.feature.read/manage`, `platform.secret.read/manage`, `platform.administer` — every controlled mutation and sensitive read authorizes one (default deny). Mirroring `m04-admin`'s `admin.*` split, **`platform.administer` is the cross-tenant control-plane permission a tenant admin never holds by default**; manage/secret codes are privileged. No `platform.admin` / wildcard bypass; request-supplied tenant/platform identifiers create no authority.

**Consequence:** the enterprise platform gains a single canonical foundation for metadata/config/feature-flags/secret-references without a second engine, with a clear M04(console)→M30(platform) consumer relationship, an explicit platform-vs-tenant permission boundary, and feature flags that can never bypass RBAC or weaken an absolute control. Rejected: a second feature-flag engine in M04; a flag that grants authorization; a tenant override of an absolute platform control; a `platform.admin` bypass; migrating a canonical capability that another module already owns (none did).

## ADR-116 — M30 owns the secret-reference SEAM only (opaque `secretref:` pointers, zero secret values); real secret/key management remains M41; M30 declares `platform.lifecycle` and exposes no REST API (Stage 6A)
**Status:** **ACCEPTED** — 2026-08-08. Module `m30-platform`, branch as ADR-115.

**Decision (secrets seam):** M30 owns **only the secret-reference abstraction** — opaque `secretref:` pointers (reusing the repository-authoritative `SECRET_REFERENCE_PATTERN` from m24), their governance metadata (purpose, scope, lifecycle active/rotated/revoked) and the association of secret-bearing configuration to a reference. M30 stores **ZERO secret values**: no password, API key, private key, token, certificate material, plaintext or encrypted credential, or decrypted value — a DB inspection proves **0 secret/credential-value columns** (a secret-bearing config value carries only a `secret_ref`, never a value). Runtime secret resolution is placed **behind a `SecretResolver` port with a deterministic offline test double only** (no production secrets adapter, no network); it **fails closed**. **Real secret/key management is M41-security (Stage 6H)** — M30 does not implement it and integrates M41 behind the port when built. **Decision (events):** M30 declares and owns the **`platform.lifecycle`** family (already registered in `event-registry.yaml`); it adds the payload + envelope alias in `contracts/platform-events.ts` and appends it to the `DomainEvent` union tail, publishes only that family through the **one m06 outbox** (no second outbox, privacy-safe payloads — ids/keys/scopes/reason codes only, never config/secret values). **Decision (API):** `naming-map` declares `api_prefixes: []`; M30 ships as an **internal governed platform library** (public service ports, opaque-id DTOs) with **no REST surface** — the admin HTTP surface belongs to M04 orchestrating over M30's contracts. Introducing `/api/v1/platform` would be a naming-governance change, not made here.

**Consequence:** the platform gains a governed secret-reference seam and a platform event family without becoming a secrets manager, without persisting a single secret value, without a second outbox, and without an ungoverned HTTP surface — M41 drops in behind the resolver port unchanged. Rejected: storing any secret value/blob in M30; an M30 secrets/key-management engine; a production secrets adapter or network egress; a second event family or outbox; a `/api/v1/platform` REST API not assigned by the naming-map.

## ADR-117 — M31 Studio is the DESIGN-TIME authoring layer (author/validate/version/review/publish/bind); the runtime engines stay canonical (M06 workflow, M07 rules); M31 is the canonical owner of reusable declarative FORMS; no arbitrary code (Stage 6B)
**Status:** **ACCEPTED** — 2026-08-08. Module `m31-studio`, branch `feature/stage-6-m31-studio`.

**Decision (Studio ownership — GAP M31-1):** M31 is a **DESIGN-TIME authoring Studio**, NOT a second runtime engine. It owns the design lifecycle — **author → validate → version → review → publish → bind** — for workflow, rule and form designs, and nothing that executes. **M06 remains the canonical workflow engine** (owns `workflow_definition`/`workflow_definition_version` + `workflow.definition.*` authoring/publish + all instance execution) and **M07 remains the canonical rules engine** (owns `rule_set`/`rule_set_version` + `rules.engine.*` authoring/publish + evaluation). A Studio **workflow design**, once validated and approved, **binds/compiles to M06's canonical `DefinitionService` contract** (`create`/`newVersion`/`validate`/`publish`, spec built to `WorkflowDefinitionSpec`); a Studio **rule design** binds to M07's `RuleSetService` contract (spec built to `RuleSetSpec`). M31 stores only the **opaque binding tuple** M06/M07 return — `(definitionId|ruleSetId, versionId, versionNumber, code, contentHash)` — and NEVER creates a `workflow_definition`/`workflow_definition_version`/`rule_set`/`rule_set_version` table, NEVER executes/transitions/schedules an instance, NEVER owns runtime state, and NEVER reads M06/M07 private tables (it calls their public services under its own permissions). This is the same consume-by-contract relationship M04-admin has with the engines it orchestrates.

**Decision (Forms ownership — GAP M31-2):** repository inspection proves **no canonical reusable Forms capability exists anywhere** — the only form-shaped table is m12's domain-specific `feedback_questionnaire`. **M31 is therefore assigned canonical ownership of reusable DESIGN-TIME form/schema definitions** (recorded in `SHARED_SERVICE_OWNERSHIP.md`). Studio forms are **declarative** platform design metadata — a form design is a governed artifact (`kind: form`) whose immutable published version carries a bounded declarative schema (typed fields, sections, validation metadata, conditional-visibility metadata, binding metadata) and **no executable code**. The load-bearing boundary is **FORM DEFINITION ≠ BUSINESS RECORD**: M31 owns the schema/definition only; it stores **no submitted form business data** — domain modules own submitted records, and m12 questionnaires stay owned by m12. M31 does not duplicate `feedback_questionnaire`.

**Decision (no arbitrary code — GAP M31-4):** Studio is **not** an arbitrary-code environment. It persists **declarative design metadata only**; there is **no** eval/`Function`/`vm`, no dynamic compilation, no arbitrary JavaScript/TypeScript/SQL/shell, no user binary, no unrestricted HTTP, no embedded credential. Where a design needs a condition/expression, M31 **reuses the existing sandboxed mechanisms owned by M06/M07** — `compileExpression` (m06, closed allow-list, hand-written parser, no eval) for workflow gateway conditions and the structured typed `Condition`/`validateRuleSet` model (m07) for rules — and **fails closed** on any unsupported/prohibited expression. Design validation is fail-closed and machine-readable (bounded reason codes): structural/graph correctness, unreachable nodes, invalid transitions, missing/unpublished-dependency bindings, invalid field types/validation, circular dependencies, unsupported capability references, attempted secret VALUES and prohibited execution expressions, and cross-tenant bindings are all rejected, and **a design that fails validation can never be published**.

**Decision (publication is a controlled action — Section F/T):** publishing a design can alter runtime behaviour, so it is a **CONTROLLED ACTION with maker-checker / SoD** — enforced in the pure gates (`isHumanActor`; `requester ≠ approver`, fail closed), the services (human-approver guard + validation-passed + current-version CAS + valid binding before publish), and the **database** (`studio_review` decider NOT NULL and `decided_by <> requested_by`; a non-rejected publish requires a passing validation). **AI never approves a Studio artifact.** A **published version is immutable** — changes require a new version (DB trigger freezes `spec`/`content_hash` at insert and forbids any mutation once a version reaches `published`/`superseded`/`archived`).

**Consequence:** the platform gains a governed, human-approved, design-time authoring Studio for workflows, rules and reusable forms that compiles/binds to the canonical engines without duplicating a single runtime engine, table or outbox, without arbitrary code, and without ever storing submitted business data — a Studio design becomes a real M06/M07 published definition through their existing contracts. Rejected: a second workflow/rules/notification/feature-flag/RBAC/audit engine; a duplicate `workflow_definition`/`rule_set` table; Studio executing/transitioning an instance; arbitrary code/expression/SQL/shell execution; a self-published or AI-approved design; a published version mutated in place; submitted form business data stored in Studio; duplicating m12 questionnaires.

## ADR-118 — M31 declares the `studio.*` permission namespace (GAP M31-3 resolved), reuses the one m06 outbox + the `studio.lifecycle` family, defers the UNBUILT M33 integration behind a fail-closed capability-catalog port, uses the M30 secret-reference seam, and exposes no REST API (Stage 6B)
**Status:** **ACCEPTED** — 2026-08-08. Module `m31-studio`, branch as ADR-117.

**Decision (permission namespace — GAP M31-3):** M31 had `permission_namespaces: []`. **Resolution:** a new three-segment **`studio.*`** namespace derived from the actual operation surface — `studio.project.read/manage`, `studio.artifact.read/author/validate/publish/archive`, `studio.binding.manage`, `studio.control.administer` — every controlled mutation and sensitive read authorizes exactly one (default deny). Mirroring `platform.*`/`admin.*`, **`studio.control.administer` is the cross-tenant control-plane permission a tenant admin never holds by default**; `manage`/`publish`/`archive`/`binding.manage`/`control.administer` are privileged. **Publish/archive/bind are privileged controlled actions.** No `studio.admin`/wildcard bypass; request-supplied identifiers create no authority; a **feature flag can never substitute RBAC** (RBAC DENY + FEATURE ENABLED = DENY — M30 stays authoritative). Forms are governed as artifacts (`kind: form`) under `studio.artifact.*` rather than a redundant `studio.form.*` split.

**Decision (integration boundary — GAP M31-5):** **M33 Integration Foundation is UNBUILT** and M31 does not implement it. If a Studio design references an integration/action capability, M31 stores only an **opaque integration capability reference** and resolves availability behind an **`IntegrationCapabilityCatalogPort`** with a deterministic offline test double and a **fail-closed `UnavailableIntegrationCatalog`** (an unknown/unavailable capability fails validation — no network, no connector execution, no credential). M33 later implements the port without changing M31's domain semantics.

**Decision (secrets — GAP M31-6):** M31 stores **no secret value** in any design. A secret-bearing design value carries only an opaque **`secretref:`** pointer validated against the repository-authoritative `SECRET_REFERENCE_PATTERN` (the M30/M24 secret-reference seam); a raw password/api-key/token/private-key/credential value is **rejected** by validation and there is **0 secret-value column** in M31 persistence.

**Decision (events + outbox):** M31 declares and owns the **`studio.lifecycle`** family (already reserved in `event-registry.yaml`); it adds `contracts/studio-events.ts` (payload + envelope alias) and appends it to the `DomainEvent` union tail + `DOMAIN_EVENT_FAMILIES`, and publishes **only design-time transitions** (artifact created, version validated, review requested, artifact published/superseded/archived) through the **one m06 outbox** (an `M31Emitter` cloning the M06/M07/M08 emitter). **No second outbox**, no runtime workflow events, privacy-safe payloads (opaque ids/kinds/versions/reason codes — never spec content, form data, secrets or credentials).

**Decision (API):** `naming-map` declares `api_prefixes: []`; M31 ships as an **internal governed platform library** (public services + ports + opaque-id DTOs) with **no REST surface** — a Studio UI/HTTP surface would be a later naming-governance change (an M04-style console orchestrating M31's contracts), not made here. `/api/v1/studio` is NOT added.

**Consequence:** M31's design surface is fully governed — a declared permission namespace with a control-plane split, one outbox and one family, an unbuilt-integration boundary that fails closed, secret references never values, and no ungoverned HTTP surface — so the real M33 and real form-runtime consumers drop in behind their ports/contracts unchanged. Rejected: `studio.admin`/wildcard; a feature flag granting authority; building M33 or executing a connector; a secret value in a design; a second outbox or event family; a studio.lifecycle event carrying spec/form/secret content; a `/api/v1/studio` REST API not assigned by the naming-map.

## ADR-119 — M32 Analytics is a GOVERNED, DERIVED/READ analytics builder (source of truth stays with source modules); a governed semantic query layer forbids arbitrary SQL; entitlement intersection means aggregation grants no access; it implements the M28 ExecutiveAnalyticsPort; m10-report is subsumed (no second reporting engine) (Stage 6C)
**Status:** **ACCEPTED** — 2026-08-10. Module `m32-analytics`, branch `feature/stage-6-m32-analytics`.

**Decision (derived/read layer + m10-report ownership):** M32 is the enterprise **Reporting & Analytics BUILDER** — governed semantic **datasets**, **metrics/KPIs**, **reports**, **dashboards**, governed **exports** and scheduled-report **metadata**. It is a **DERIVED/READ layer**: every operational/business record stays owned by its source module; M32 owns only **derived, read-only, REBUILDABLE** projections (`analytics_materialization`, append-only per generation) with **mandatory LINEAGE** (`analytics_lineage`: authoritative source module, dataset, metric version, extraction window, filters, classification), and performs **NO business mutation** (no posting/payment/reconciliation/approval/workflow execution). `SHARED_SERVICE_OWNERSHIP.md` already co-lists **m10-report** (Stage 2, mvp:true, 0 tables, never built) and m32; this ADR resolves that overlap — **M32 is the canonical reporting/analytics builder**, m10-report is subsumed (there is **no second reporting engine**). M32 owns analytics **dashboards** as governed report definitions; it does **not** create a second generic saved-view engine — **m04's `admin_saved_view`** (admin-console filter presets) stays m04's.

**Decision (NO arbitrary SQL — governed semantic query):** the only way to query is `compileMetricQuery`, which references **WHITELISTED** dataset dimensions/measures + **WHITELISTED** operators (`eq/neq/gt/gte/lt/lte/in/between`) and returns a **STRUCTURED plan** (never an SQL string); the repository maps the plan's whitelisted identifiers to a **fixed parameterized SELECT** over the metric's latest materialization and the compiled scalar filters are applied to already-aggregated rows — **a user string is never concatenated into SQL**. There is **no** eval/`Function`/`vm`/shell/dynamic code/unrestricted table access; an unknown dimension/measure/operator or a non-scalar value **FAILS CLOSED** (a 403 governance refusal). **Decision (RLS + entitlement survive aggregation):** every tenant table is **FORCE-RLS**; `evaluateEntitlement` requires the caller to hold **EVERY** required entitlement (intersection) at sufficient **scope** and **sensitivity** before any aggregate is returned — a denied caller gets **no partial aggregate** (no hidden-count leakage), and **aggregation never grants access** the caller lacks, nor enables cross-tenant inference (RLS). **Decision (M28 port):** `M32ExecutiveAnalyticsAdapter` **implements M28's `ExecutiveAnalyticsPort`** (ADR-112) — read-only, bounded, tenant-scoped, entitlement-filtered, citation-bearing (source module/opaque metric ref/version/location, a non-restricted headline that carries **no metric value**); **M28 is the downstream consumer** and is not modified. **Decision (controlled publication):** publishing a metric/report is a **controlled action** (maker-checker/SoD — a human approver who is not the requester; `analytics_review` `decided_by <> requested_by`; a metric/report cannot reach validated/review/published without a passing validation; a **published definition is immutable** via trigger, only moving to superseded). **Decision (money):** metric values are **bigint minor units / exact `numeric` decimal / integer basis points** with explicit currency — there is **NO float column**, and a money metric must declare its currency (no silent cross-currency aggregation). **Decision (reuse, no second engine):** M32 reuses m02 RBAC, m03 audit, m06 workflow/timers/**outbox**, m08 notifications, m09 documents (export bytes behind an opaque `docref:`), m24 AI and m30 platform **by contract**; scheduled reports hold **opaque m06 timer + m08 notify references** (m32 owns no scheduler/timer/notify engine); it declares `/api/v1/analytics`, the `analytics.*` namespace (12 codes, 6 privileged), the `ANALYTICS_` prefix (17 codes) and the **`analytics.lifecycle`** family, publishing through the **one m06 outbox** (no second outbox). `m33-integration` + `m41-security` are **UNBUILT** and deferred behind a **fail-closed `MaterializationSourcePort`** (deterministic offline double; no network). `reference_tables` **42 → 11** governed core (documented). Events carry ids/keys/kinds/versions/**row counts**/reason codes only — never a metric value, a report body, source data or a secret.

**Consequence:** the platform gains a governed enterprise analytics builder — semantic metrics, reports, dashboards, exports and schedules — that never becomes a source of truth, never runs arbitrary SQL, never lets aggregation widen access, preserves lineage for every result M28 cites, keeps money exact, and reuses every canonical engine — so a real source adapter, the real m33 integration and the real m41 security drop in behind their ports unchanged. Rejected: M32 owning/mutating operational data; arbitrary SQL/eval/shell/unrestricted table access; an aggregate returned to an unentitled caller or a hidden-count leak; a second reporting/query/scheduler/notification/RBAC/audit/outbox engine or a second saved-view store; a float money column or silent cross-currency aggregation; a metric/report published without maker-checker or mutated after publication; an analytics event carrying a value/report body/source data/secret; building m33 or m41.
