...
| 4 | Authorize the external penetration-test engagement | Engage a qualified independent external provider under NDA/CoI against the isolated staging stack (pack: `PRODUCTION_PENTEST_PROVIDER_PACK.md`) | MD + Head of Risk | Day 1 (long lead) | G1 (non-waivable) cannot start; blocks GO | Signed engagement + provider report + Auditor assurance | ◑ ACTIVATION IN PROGRESS — records authored (`G1_PENTEST_ACTIVATION_RECORD.md` + provider/auditor independence declarations); signatures + isolated instance pending |
