...
| **M12 Feedback** | DAY-1 CRITICAL | **ACCEPTED — authenticated browser + backend proven** | ✓ `api-feedback` (27) | ✓ create/activity/escalate/resolve(SoD)/close/reopen | **Closure pass:** full workflow incl. resolution **submit→approve SoD** (cs_hod≠cs_manager); close rule-gated; **D-M12-1 fixed** (approve button gated on wrong status → had blocked all approvals). See `DAY1_ACCEPTANCE_CLOSURE_REPORT.md` |
| **M13 Cases** | DAY-1 CRITICAL | **ACCEPTED — authenticated browser + backend proven** | ✓ `api-cases` (18) | ✓ create/triage/decision+approve(SoD)/task/activity/lifecycle | **Browser-verified (final campaign):** create/party+`[redacted]`/decision submit→approve **SoD (distinct identities)**/activity/lifecycle — see `FINAL_DAY1_BROWSER_ACCEPTANCE_REPORT.md` |
| **M14 Legal Matters** | DAY-1 CRITICAL | **PARTIALLY ACCEPTED** (maker browser-proven) | ✓ `api-legal` (29) | ✓ create/court-events/pleadings/costs/appeal/lifecycle | Browser-verified: create/cost `123456`/court-event/pleading/appeal; settlement-approve + close (checker) pending |
| **M16 Litigation** | DAY-1 CRITICAL | **PARTIALLY ACCEPTED** (maker browser-proven) | ✓ `api-litigation` (27) | ✓ witnesses/exhibits/orders/obligations/bundles/filing SoD | Browser-verified: proceeding/witness/exhibit/order/bundle; filing+bundle approve (SoD) pending; locked-state server-enforced |
...
| **M19 Finance** | DAY-1 CRITICAL | **PARTIALLY ACCEPTED** (entity create browser-proven) | ✓ `api-finance` (16) | ✓ entity CRUD+lifecycle, fiscal-year/period, GL account/CoA | Browser-verified: accounting-entity create (`FIN_ENTITY_REGISTERED`); deactivate + fiscal-year/period BLOCKED by automation date/confirm-input (no product defect); catalog master-data edit = DEFERRED |
...
| **M21 Journals** | DAY-1 CRITICAL | **PARTIALLY ACCEPTED** (maker browser-proven) | ✓ `api-journals` (14) | ✓ draft/header-edit/lines/validate→submit→authorize-post(SoD, no auto-post)/withdraw | Closure pass: create→lines→validate→submit proven; posting-authorize pending. **D-M21-1/2 fixed, D-M21-3 open** |
| **M22 Approvals & Delegations** | DAY-1 CRITICAL | **PARTIALLY ACCEPTED** (approval SoD proven) | ✓ (approvals specs) | ✓ approve/reject/return/escalate + delegation grant/list/revoke | Closure pass: **approval-decision SoD proven** (decider≠requester); delegation grant BLOCKED (client anomaly); **D-M22-1 fixed** |
