...
**Build baseline:** `main` @ `cd29b7b5b1c0220fd801989e528d06911395b7a9` (Stage 2.1 m03 merged PR #9 + certified PR #11). Planning-time baseline was `56b7d3e`.
...
| Fact | Value |
| --- | --- |
| Certified build baseline | `cd29b7b` (Stage 2.1 m03-audit merged PR #9 → `587a3ce`, certified PR #11 → `cd29b7b`) |
| m03-audit | **merged + certified** on main; `AUDIT` bound to persistent `AuditService` |
| m06-workflow | placeholder README only (18 reference tables baseline); manifest `status: documented` — this stage builds it |
| m08 / m09 / m12 / m13 / m22 | placeholder READMEs only — no implementation (and none in this stage) |
| Reserved m06 axes | API `/api/v1/workflow`, perms `workflow.*`, event family `workflow.lifecycle` (GAP-1: unregistered), audit `WORKFLOW_` |

### Pre-build decisions — RESOLVED (2026-07-23)
- **OD-1 (RESOLVED):** API prefix is the repository-reserved **`/api/v1/workflow`** (singular), matching `naming-map.yaml`; resource segments are plural (`/workflow/definitions`, `/workflow/instances`, `/workflow/tasks`). Not `/api/v1/workflows`.
- **OD-4 (RESOLVED):** Use the existing authoritative entity-type convention. **No new enterprise `entity_type_registry` table is created by m06.** m06 references org nodes by their m01 `(tenant_id, id)` ids and its own workflow subject-type strings; it documents this minimal dependency rather than inventing a cross-cutting registry.
- **OD-2 / m22 boundary (RESOLVED):** **m06** owns generic approval-task orchestration, routing, decision recording, quorum/sequential/parallel mechanics, maker-checker, and SoD enforcement, configured by the workflow **version** (local approval config). **m22-approval** (not built here) will own enterprise approval-policy administration, reusable approval matrices, approval limits, delegations-of-authority, and governed policy config; m06 may consume m22 contracts later but must remain fully usable with local workflow-version approval config until m22 exists. **No m22 runtime code in this stage.**
