/**
 * Recovery SLA policy spec + deterministic SLA calculation. A policy is a versioned, immutable-after-publish
 * document (mirrors m09 doctype / m16). Due dates and warn/breach state are computed as PURE functions of a
 * supplied clock (epoch ms) + accumulated paused duration — there is no ambient `Date.now`, so SLA behaviour is
 * fully testable and replayable (ADR-070). m17 does NOT build a timer engine; it uses these deterministic
 * calculations and delegates timer dispatch / escalation to m06 / m08.
 */
import { RecoveryError } from './limits.ts';

export const SLA_POLICY_SCHEMA_VERSION = 1;

export interface RecoverySlaPolicySpec {
  readonly schemaVersion: number;
  readonly code: string;
  readonly name: string;
  readonly ackMinutes: number;
  readonly strategyMinutes: number;
  readonly demandMinutes: number;
  readonly responseMinutes: number;
  readonly arrangementReviewMinutes: number;
  readonly enforcementFilingMinutes: number;
  readonly agentReportMinutes: number;
...
const MINUTE_FIELDS = [
  ...
  'demandMinutes',
  'responseMinutes',
  'arrangementReviewMinutes',
  'enforcementFilingMinutes',
  'agentReportMinutes',
  'reviewMinutes',
  'closureMinutes',
] as const;

export function validateRecoverySlaPolicySpec(candidate: unknown): SlaSpecValidation {
...
export interface SlaDueDates {
  ...
  readonly strategyAtMs: number;
  readonly demandAtMs: number;
  readonly responseAtMs: number;
  readonly arrangementReviewAtMs: number;
  readonly enforcementFilingAtMs: number;
  readonly agentReportAtMs: number;
...
export function computeDueDates(policy: RecoverySlaPolicySpec, startMs: number): SlaDueDates {
  if (!Number.isFinite(startMs)) throw new RecoveryError('INVALID_START', 'SLA start is invalid');
  return {
    ackAtMs: startMs + policy.ackMinutes * MIN_MS,
    strategyAtMs: startMs + policy.strategyMinutes * MIN_MS,
    demandAtMs: startMs + policy.demandMinutes * MIN_MS,
    responseAtMs: startMs + policy.responseMinutes * MIN_MS,
    arrangementReviewAtMs: startMs + policy.arrangementReviewMinutes * MIN_MS,
    enforcementFilingAtMs: startMs + policy.enforcementFilingMinutes * MIN_MS,
    agentReportAtMs: startMs + policy.agentReportMinutes * MIN_MS,
    reviewAtMs: startMs + policy.reviewMinutes * MIN_MS,
