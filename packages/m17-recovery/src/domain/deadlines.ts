/**
 * Deterministic recovery-deadline calculation — PURE. Deadlines (demand_response / arrangement_review /
 * installment_due / enforcement_filing / security_realization / agent_report / statutory_period / limitation / …)
 * are computed from a start instant + a calculation rule against a SUPPLIED clock (epoch ms), never ambient
 * `Date.now`, so behaviour is replayable and unit-testable (ADR-070). Two rule kinds are supported
 ...
 * high-risk and clearly distinguishable. m17 builds NO timer engine; breach dispatch is delegated to m06/m08.
 */
import { RecoveryError, isDeadlineType } from './limits.ts';
...
export function computeDeadlineDueMs(input: DeadlineInput): number {
  if (!isDeadlineType(input.type))
    throw new RecoveryError('INVALID_DEADLINE_TYPE', `unknown deadline type "${input.type}"`);
  if (!Number.isFinite(input.startMs)) throw new RecoveryError('INVALID_START', 'deadline start is invalid');
  if (input.rule.kind === 'explicit') {
    if (typeof input.rule.dueMs !== 'number' || !Number.isFinite(input.rule.dueMs)) {
      throw new RecoveryError('INVALID_RULE', 'explicit deadline needs a finite dueMs');
    }
    return input.rule.dueMs;
  }
  const days = input.rule.days;
  if (typeof days !== 'number' || !Number.isInteger(days) || days < 0) {
    throw new RecoveryError('INVALID_RULE', 'offset_days deadline needs a non-negative integer days');
  }
