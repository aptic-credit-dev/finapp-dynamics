/**
 * Case SLA policy spec + deterministic SLA calculation (F17). A policy is a versioned, immutable-after-publish
 * document (mirrors m09 doctype / m12 sla). Due dates and warn/breach state are computed as PURE functions of a
 * supplied clock (epoch ms) + accumulated paused duration — there is no ambient `Date.now`, so SLA behaviour is
 * fully testable and replayable (ADR-058). m13 does NOT build a timer engine; it uses these deterministic
 * calculations and delegates timer dispatch / escalation to m06 / m08.
 */
import { CaseError } from './limits.ts';

export const SLA_POLICY_SCHEMA_VERSION = 1;

export interface CaseSlaPolicySpec {
  readonly schemaVersion: number;
  readonly code: string;
  readonly name: string;
  readonly ackMinutes: number;
  readonly triageMinutes: number;
  readonly assignMinutes: number;
  readonly investigationMinutes: number;
...
export function validateCaseSlaPolicySpec(candidate: unknown): SlaSpecValidation {
...
export interface SlaDueDates {
  ...
  readonly triageAtMs: number;
  readonly assignAtMs: number;
  readonly investigationAtMs: number;
...
export function computeDueDates(policy: CaseSlaPolicySpec, startMs: number): SlaDueDates {
  if (!Number.isFinite(startMs)) throw new CaseError('INVALID_START', 'SLA start is invalid');
  return {
    ackAtMs: startMs + policy.ackMinutes * MIN_MS,
    triageAtMs: startMs + policy.triageMinutes * MIN_MS,
    assignAtMs: startMs + policy.assignMinutes * MIN_MS,
    investigationAtMs: startMs + policy.investigationMinutes * MIN_MS,
 ...
 * elapsed working time has passed `warnThresholdPct` of the window; `breached` once the (pause-adjusted) due time
 * is passed. PURE — the caller supplies `nowMs` and accumulated `pausedMs`.
 */
