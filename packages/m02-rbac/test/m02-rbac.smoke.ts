...
export default defineSuite('m02-rbac', async (t) => {
  ...
  t.deepEqual(
    ...
    'the role statuses are the four ADR-017 states',
  );
  ...
  t.ok(
    !checkRoleTransition('active', 'suspend').allowed ||
      checkRoleTransition('active', 'suspend', { reason: 'x' }).allowed,
    'suspend needs a reason',
  );
  t.ok(!checkRoleTransition('active', 'suspend').allowed, 'suspend without a reason is refused');
  t.equal(
    checkRoleTransition('active', 'suspend', { reason: 'policy' }).to,
    'suspended',
    'active + reason -> suspended',
  );
  t.equal(checkRoleTransition('suspended', 'reactivate').to, 'active', 'suspended -> reactivate -> active');
  t.equal(
    checkRoleTransition('active', 'retire', { reason: 'eol' }).to,
    'retired',
    'active -> retire -> retired',
  );
  t.ok(
    !checkRoleTransition('active', 'reactivate').allowed,
    'active -> reactivate is not a legal transition',
  );
  t.ok(
    !checkRoleTransition('retired', 'activate').allowed,
    'a retired role is terminal — nothing reopens it',
  );
  ...
  t.ok(
    !roleGrantsPermissions('draft') &&
      !roleGrantsPermissions('suspended') &&
      !roleGrantsPermissions('retired'),
    'a draft/suspended/retired role grants nothing (fail closed)',
  );
  ...
  t.deepEqual(
    ...
    'the assignment statuses',
  );
  t.ok(
    isAssignmentStatus('revoked') && !isAssignmentStatus('deleted'),
    'isAssignmentStatus admits only real statuses',
  );
  t.equal(
    checkAssignmentTransition('active', 'suspend', { reason: 'leave' }).to,
    'suspended',
    'active -> suspend -> suspended',
  );
  t.equal(
    checkAssignmentTransition('active', 'revoke', { reason: 'offboard' }).to,
    'revoked',
  ...
  t.ok(
    !assignmentIsEffective('active', now, null, now),
    'an assignment at its exact expiry yields nothing (half-open)',
  );
  t.ok(
    assignmentIsEffective('active', now, now, now + 1),
    'effective_from is inclusive, expires_at exclusive',
  );

  // --- scope algebra ---------------------------------------------------------------------------------
  t.deepEqual(
    [...SCOPE_LEVELS],
    ['platform', 'tenant', 'entity', 'branch', 'department'],
    'the MVP scope levels',
  );
  t.ok(isScopeLevel('branch') && !isScopeLevel('galaxy'), 'isScopeLevel admits only real levels');

  t.ok(
    assignmentScopeContains({ level: 'platform', ref: null }, { level: 'department', ref: 'n1' }),
    'platform contains everything',
  );
  t.ok(
    assignmentScopeContains({ level: 'tenant', ref: null }, { level: 'branch', ref: 'n1' }),
    'a tenant-wide assignment contains any in-tenant scope',
  );
  t.ok(
    assignmentScopeContains({ level: 'entity', ref: 'n1' }, { level: 'entity', ref: 'n1' }),
    'an org assignment contains its exact node',
  );
  t.ok(
    !assignmentScopeContains({ level: 'entity', ref: 'n1' }, { level: 'entity', ref: 'n2' }),
    'but not a different node (default deny)',
  );
  t.ok(
    !assignmentScopeContains({ level: 'entity', ref: 'n1' }, { level: 'branch', ref: 'n1' }),
    'nor a different level with the same ref',
  );
  ...
  t.equal(
    ALL_RBAC_PERMISSIONS.length,
    Object.keys(RBAC_PERMISSIONS).length,
    'every declared permission is exported once',
  );
  ...
  t.equal(
    new Set(ALL_RBAC_AUDIT_CODES).size,
    Object.keys(RBAC_AUDIT_CODES).length,
    'no duplicate audit codes',
  );
  ...
  const sysCtx: SystemContext & { permissions: readonly string[] } = {
    reason: 'x',
    correlationId: 'c3',
    permissions: ['rbac.sod.manage'],
  };
  ...
  t.ok(
    await authz.can(sysCtx, 'rbac.sod.manage'),
    'a system context answers from the permissions it carries',
  );
  t.ok(
    !(await authz.can(bareSys, 'rbac.sod.manage')),
    'a system context with NO permissions is not a universal allow',
  );
