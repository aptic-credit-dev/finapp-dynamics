...
export class AssignmentService {
  ...
  async grant(
    ...
    actor: string,
    input: {
      membershipId: string; roleId: string;
      scopeLevel?: string; scopeRef?: string | null;
      effectiveFrom?: Date | null; expiresAt?: Date | null; justification?: string | null;
    ...
    if (scopeLevel !== 'tenant' && scopeRef === null) throw badRequest('an organizational scope requires a scope reference.', ctx.correlationId);
    ...
    return this.db.withTenant(ctx, async (tx) => {
      ...
      if (role === null) throw badRequest('roleId does not exist or is not visible in this tenant.', ctx.correlationId);
      if (role.status !== 'active') throw ProblemError.conflict(`Cannot assign a ${role.status} role.`, ctx.correlationId);

      const membership = await this.repo.findMembership(tx, input.membershipId);
      if (membership === null) throw badRequest('membershipId does not exist in this tenant.', ctx.correlationId);
      if (membership.status === 'ended') throw ProblemError.conflict('Cannot assign a role to an ended membership.', ctx.correlationId);
      ...
      if (beyond !== undefined) {
        throw ProblemError.forbidden('You cannot grant a role that includes permissions you do not hold.', ctx.correlationId);
      }
      ...
      const conflict = await this.sod.firstConflict(tx, { membershipId: input.membershipId, newRoleId: input.roleId, newRoleCode: role.code, currentRoleCodes });
      if (conflict !== null) await this.sod.rejectConflict(tx, sys, { conflict, membershipId: input.membershipId, correlationId: ctx.correlationId, actor });

      let row: AssignmentRow;
      try {
        row = await this.repo.insertAssignment(tx, {
          tenantId: ctx.tenantId, membershipId: input.membershipId, identityId: membership.identity_id, roleId: input.roleId,
          scopeLevel, scopeRef, effectiveFrom: input.effectiveFrom ?? null, expiresAt: input.expiresAt ?? null,
          justification: input.justification ?? null, grantedBy: actor,
        });
      } catch (error: unknown) {
  ...
  async applyAction(ctx: RequestContext, actor: string, id: string, action: AssignmentAction, opts: { reason?: string; expectedVersion: number }): Promise<AssignmentRow> {
    ...
    return this.db.withTenant(ctx, async (tx) => {
      ...
      const check = checkAssignmentTransition(current.status as AssignmentStatus, action, { reason: opts.reason });
      ...
      await this.emitter.publish(tx, evt, ctx.tenantId, ctx.correlationId, actor, { assignmentId: id, roleId: current.role_id, subjectId: current.membership_id, fromStatus: current.status, toStatus: check.to, ...(opts.reason === undefined ? {} : { reason: opts.reason }) });
      return updated;
    });
  }
}
