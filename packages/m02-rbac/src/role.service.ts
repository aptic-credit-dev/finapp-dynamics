...
export class RoleService {
  ...
  async create(
    ctx: RequestContext,
    actor: string,
    input: { code: string; name: string; description?: string | null; risk?: string },
  ): Promise<RoleRow> {
    await this.authz.require(ctx, RBAC_PERMISSIONS.roleCreate);
    if (!ROLE_CODE_PATTERN.test(input.code))
      throw badRequest('code must be lower_snake_case (3-64 chars).', ctx.correlationId);
    if (typeof input.name !== 'string' || input.name.trim() === '')
      throw badRequest('name is required.', ctx.correlationId);
    const risk = input.risk ?? 'normal';
    if (!['normal', 'elevated', 'critical'].includes(risk))
      throw badRequest('invalid risk.', ctx.correlationId);
    return this.db.withTenant(ctx, async (tx) => {
      let row: RoleRow;
      try {
        row = await this.repo.insertRole(tx, {
        ...
        if (isUniqueViolation(error))
          throw ProblemError.conflict(
            `Role code "${input.code}" already exists in this tenant.`,
            ctx.correlationId,
          );
        throw error;
      }
      await this.repo.appendRoleHistory(tx, {
        tenantId: ctx.tenantId,
        roleId: row.id,
        fromStatus: null,
        toStatus: 'draft',
      ...
      await this.emitter.recordAudit(tx, ctx, {
        code: RBAC_AUDIT_CODES.roleCreated,
        entityType: 'role',
        entityId: row.id,
        detail: { code: row.code },
      });
      await this.emitter.publish(tx, 'RoleCreated', ctx.tenantId, ctx.correlationId, actor, {
        roleId: row.id,
        roleCode: row.code,
        toStatus: 'draft',
      });
  ...
  async update(
    ...
    return this.db.withTenant(ctx, async (tx) => {
      ...
      const updated = await this.repo.updateRoleMeta(tx, {
        id,
      ...
      await this.emitter.recordAudit(tx, ctx, {
        code: RBAC_AUDIT_CODES.roleUpdated,
        entityType: 'role',
        entityId: id,
      });
      await this.emitter.publish(tx, 'RoleUpdated', ctx.tenantId, ctx.correlationId, actor, {
        roleId: id,
  ...
  async applyAction(
    ...
    return this.db.withTenant(ctx, async (tx) => {
      ...
      const updated = await this.repo.applyRoleStatus(tx, {
        id,
  ...
  async changePermissions(
    ...
    if (add.length === 0 && remove.length === 0)
      throw badRequest('Provide permissions to add or remove.', ctx.correlationId);
    ...
    for (const code of add) {
      if (!grantorSet.has(code))
        throw ProblemError.forbidden(
          `You cannot grant a permission you do not hold: ${code}.`,
          ctx.correlationId,
        );
    }
    return this.db.withTenant(ctx, async (tx) => {
      ...
      let removed = 0;
      for (const code of add) {
        if (!(await this.repo.permissionExists(tx, code, true)))
          throw badRequest(`Unknown or non-tenant-assignable permission: ${code}.`, ctx.correlationId);
        if (
          await this.repo.addRolePermission(tx, {
            roleId: id,
            tenantId: ctx.tenantId,
            code,
            grantedBy: actor,
      ...
      await this.emitter.recordAudit(tx, ctx, {
        code: RBAC_AUDIT_CODES.rolePermissionsChanged,
        entityType: 'role',
        entityId: id,
        detail: { added, removed },
      });
      await this.emitter.publish(tx, 'RolePermissionsChanged', ctx.tenantId, ctx.correlationId, actor, {
        roleId: id,
        added,
        removed,
      });
