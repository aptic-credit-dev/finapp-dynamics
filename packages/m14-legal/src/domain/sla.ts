/**
 * Legal SLA policy spec + deterministic SLA calculation (G18). A policy is a versioned, immutable-after-publish
 * document (mirrors m09 doctype / m13). Due dates and warn/breach state are computed as PURE functions of a
 * supplied clock (epoch ms) + accumulated paused duration — there is no ambient `Date.now`, so SLA behaviour is
 * fully testable and replayable (ADR-062). m14 does NOT build a timer engine; it uses these deterministic
 * calculations and delegates timer dispatch / escalation to m06 / m08.
 */
import { LegalError } from './limits.ts';

export const SLA_POLICY_SCHEMA_VERSION = 1;

export interface LegalSlaPolicySpec {
  readonly schemaVersion: number;
  readonly code: string;
  readonly name: string;
  readonly ackMinutes: number;
  readonly reviewMinutes: number;
  readonly opinionMinutes: number;
  readonly pleadingMinutes: number;
...
export function validateLegalSlaPolicySpec(candidate: unknown): SlaSpecValidation {
...
export interface SlaDueDates {
  ...
  readonly opinionAtMs: number;
  readonly pleadingAtMs: number;
...
export function computeDueDates(policy: LegalSlaPolicySpec, startMs: number): SlaDueDates {
  if (!Number.isFinite(startMs)) throw new LegalError('INVALID_START', 'SLA start is invalid');
  return {
    ackAtMs: startMs + policy.ackMinutes * MIN_MS,
    reviewAtMs: startMs + policy.reviewMinutes * MIN_MS,
    opinionAtMs: startMs + policy.opinionMinutes * MIN_MS,
    pleadingAtMs: startMs + policy.pleadingMinutes * MIN_MS,
