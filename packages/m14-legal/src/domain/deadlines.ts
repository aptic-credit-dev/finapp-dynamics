/**
 * Deterministic legal-deadline calculation (G17) — PURE. Deadlines (filing / service / response / limitation /
 * appeal / …) are computed from a start instant + a calculation rule against a SUPPLIED clock (epoch ms), never
 * ambient `Date.now`, so behaviour is replayable and unit-testable (ADR-062). Two rule kinds are supported
 ...
 * high-risk and clearly distinguishable. m14 builds NO timer engine; breach dispatch is delegated to m06/m08.
 */
import { LegalError, isDeadlineType } from './limits.ts';
...
export function computeDeadlineDueMs(input: DeadlineInput): number {
  if (!isDeadlineType(input.type))
    throw new LegalError('INVALID_DEADLINE_TYPE', `unknown deadline type "${input.type}"`);
  if (!Number.isFinite(input.startMs)) throw new LegalError('INVALID_START', 'deadline start is invalid');
  if (input.rule.kind === 'explicit') {
    if (typeof input.rule.dueMs !== 'number' || !Number.isFinite(input.rule.dueMs)) {
      throw new LegalError('INVALID_RULE', 'explicit deadline needs a finite dueMs');
    }
    return input.rule.dueMs;
  }
  const days = input.rule.days;
  if (typeof days !== 'number' || !Number.isInteger(days) || days < 0) {
    throw new LegalError('INVALID_RULE', 'offset_days deadline needs a non-negative integer days');
  }
...
/** `limitation` is the highest-risk deadline: a matter's limitation date must not already be in the past. */
