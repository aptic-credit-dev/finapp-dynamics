/**
 * The AI-gateway PORT through which M26 consumes the M24 governed AI pipeline BY CONTRACT — it defines the small surface
 * Legal AI needs (generate a governed analysis output; drive its human review) and NOTHING about providers, routing,
 * DLP, prompts or vectors. The default adapter, `M24AiGateway`, wraps M24's `RequestService` + `ReviewService` — so
 * provider selection, DLP, approved-provider routing, confidence, citations, usage/cost and the human-review CHECK all
 * live in M24 (never duplicated, never bypassed). M26 holds only OPAQUE m24 request/output ids and never touches M24's
 * private tables or provider credentials. There is NO provider adapter, NO network, NO HTTP client and NO SDK here.
 */
...
/** The default gateway: M24's own governed services. M26 adds no AI capability of its own. */
