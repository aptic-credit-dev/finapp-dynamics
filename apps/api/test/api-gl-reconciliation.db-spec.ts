...
import { ALL_M20_PERMISSIONS, M20_PERMISSIONS } from '@finapp/m20-glrecon';

/**
 * THE GL-RECONCILIATION API, OVER HTTP, END TO END (Stage 3). Boots the real AppModule — including
 * GlReconciliationModule + the m06 outbox m20 publishes through — and drives `/api/v1/gl-reconciliation`: register a
 * GL account -> create + publish a ruleset -> import GL (with the balance invariant) + source lines (INTEGER MINOR
 * UNITS) -> create a run -> execute deterministic matching + balance -> read the auto-proposed match + confirm it ->
 * create a DRAFT journal recommendation; amounts read back as STRINGS; plus fail-closed 401 (no auth) + 403 (forged
 * permission header) + cross-tenant isolation (RLS -> 404).
 */
...
export default defineDbSpec('api-gl-reconciliation', async (ctx, t) => {
  ...
  try {
    const anon = await client('POST', '/gl-reconciliation/accounts', {
      body: { code: 'X', name: 'Y' },
    });
    t.equal(anon.status, 401, 'an anonymous caller cannot register a GL account (401)');

    const admin = await seedActor(ctx, 'gladmin');
    await grantRole(ctx, admin, ALL_M20_PERMISSIONS, 'gladmin');
    const auth = await login(client, admin);

    const acct = await client('POST', '/gl-reconciliation/accounts', {
      headers: auth.headers,
      body: { code: 'GL-1000', name: 'Cash', normalSide: 'debit' },
    });
    t.ok(
      acct.status === 200 || acct.status === 201,
      `register a GL account over HTTP (got ${String(acct.status)})`,
    );
    const acctId = String(acct.body['id']);

    const rs = await client('POST', '/gl-reconciliation/rulesets', {
      headers: auth.headers,
      body: { code: 'default', dateWindowDays: 5, amountToleranceMinor: 0, requireOppositeDirection: true },
    });
    const rsId = String(rs.body['id']);
    await client('POST', `/gl-reconciliation/rulesets/${rsId}/rules`, {
      headers: auth.headers,
    ...
    const pub = await client('POST', `/gl-reconciliation/rulesets/${rsId}/publish`, {
      headers: auth.headers,
      body: { expectedVersion: rs.body['version'] },
    });
    t.equal(pub.body['status'], 'active', 'a matching ruleset publishes over HTTP');

    const glImp = await client('POST', '/gl-reconciliation/gl-imports', {
      headers: auth.headers,
      body: {
        glAccountId: acctId,
        sourceFormat: 'csv',
        ...
        lines: [{ txnDate: '2026-01-10', amountMinor: 10000, direction: 'debit', reference: 'INV-001' }],
      },
    });
    t.ok(glImp.status === 200 || glImp.status === 201, 'import GL lines over HTTP');
    const balance = glImp.body['balance'] as Record<string, unknown> | null;
    t.ok(
      balance !== null && balance['closingBalanceMinor'] === '10000',
      'the GL balance reads back as a STRING (minor units, not a float)',
    );

    await client('POST', '/gl-reconciliation/source-imports', {
      headers: auth.headers,
      body: {
        glAccountId: acctId,
        sourceFormat: 'api',
    ...
    const run = await client('POST', '/gl-reconciliation/runs', {
      headers: auth.headers,
      body: { glAccountId: acctId, rulesetId: rsId, openingBalanceMinor: 0, closingBalanceMinor: 10000 },
    });
    const runId = String(run.body['id']);
    const executed = await client('POST', `/gl-reconciliation/runs/${runId}/execute`, {
      headers: auth.headers,
      body: { expectedVersion: run.body['version'] },
    });
    t.equal(
      executed.body['status'],
      'review_required',
      'executing advances the run to review_required over HTTP',
    );

    const runBalances = await client('GET', `/gl-reconciliation/runs/${runId}/balances`, {
      headers: auth.headers,
    });
    const rb = (runBalances.body['balances'] as Record<string, unknown>[])[0];
    t.ok(
      rb?.['calculatedClosingMinor'] === '10000' && rb['balanced'] === true,
      'the balance invariant reads back deterministically (minor units, balanced)',
    );

    const matches = await client('GET', `/gl-reconciliation/runs/${runId}/matches`, {
      headers: auth.headers,
    });
    ...
    if (proposed !== undefined) {
      const confirm = await client('POST', `/gl-reconciliation/matches/${String(proposed['id'])}/confirm`, {
        headers: auth.headers,
    ...
    t.ok(
      (rec.status === 200 || rec.status === 201) &&
        rec.body['isDraft'] === true &&
        rec.body['status'] === 'proposed',
      'a DRAFT journal recommendation is created (never posted)',
    );

    const outsider = await seedActor(ctx, 'gloutsider');
    const outsiderAuth = await login(client, outsider);
    const forged = await client('POST', '/gl-reconciliation/accounts', {
      headers: { ...outsiderAuth.headers, 'x-permissions': M20_PERMISSIONS.accountManage },
      body: { code: 'no', name: 'no' },
    });
    t.equal(forged.status, 403, 'an unprivileged actor is refused even with an x-permissions header (403)');

    const otherAdmin = await seedActor(ctx, 'globstacle');
    await grantRole(ctx, otherAdmin, ALL_M20_PERMISSIONS, 'globstacle');
    const otherAuth = await login(client, otherAdmin);
    const otherList = await client('GET', '/gl-reconciliation/accounts', { headers: otherAuth.headers });
    t.equal(
      (otherList.body['accounts'] as unknown[]).length,
      0,
      'another tenant sees none of the first tenant GL accounts (RLS)',
    );
    const otherGet = await client('GET', `/gl-reconciliation/runs/${runId}`, { headers: otherAuth.headers });
